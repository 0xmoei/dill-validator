...
while read line; do
    ...
    flag=${kv[0]}
    port=${kv[1]}
    if [ "$flag" == "monitoring-port" ]; then
        monitoring_port="$port"
    fi
    if [ "$flag" == "grpc-gateway-port" ]; then
        grpc_gateway_port="$port"
    fi
done < $default_port_file

peers_count=`curl -s localhost:$monitoring_port/p2p | grep -m 1  peers | awk '{print $1}'`
...
syncing_res=`curl -s localhost:$grpc_gateway_port/eth/v1/node/syncing`
